...
class LinearRegression:
    ...
    def fit(self, records, label,a=0.001,lamb=1,epsilon=0.00001):
        ...
        for i in range(0,10000):
            ...
            loss = np.mean((self.h(X) - y)**2)
            if abs(loss - self.loss_old) < epsilon:
            ...
            weight_new = self.weight - a*np.divide(self.cost(X,y) + self.regularization(self.weight,lamb), X.shape[0])
            # update weights
